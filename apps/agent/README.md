...
# LiveKit Agents Starter - Bun

A complete starter project for building voice AI apps with [LiveKit Agents for JavaScript](https://github.com/livekit/agents-js) and [LiveKit Cloud](https://cloud.livekit.io/).
...
Sign up for [LiveKit Cloud](https://cloud.livekit.io/) then set up the environment by copying the repository root `.env.example` to `../../.env` and filling in the required keys:
...
Use the quick feedback loop first:
...
`bun run test` runs the offline runtime smoke suite. `bun run preflight:strict` exits non-zero when the root `.env` is missing `LIVEKIT_URL`, `LIVEKIT_API_KEY`, or `LIVEKIT_API_SECRET`.

Before your first LiveKit worker run, you must download certain models such as [Silero VAD](https://docs.livekit.io/agents/logic/turns/vad/) and the [LiveKit turn detector](https://docs.livekit.io/agents/logic/turns/turn-detector/):
...
1. **Check in your `bun.lock`**: Commit this file to your repository for reproducible builds and proper configuration management. (The same applies to `livekit.toml`, if you run your agents in LiveKit Cloud)
